...
def get_order_delivery_fee(order: dict) -> float:
    fee = order.get("delivery_fee")
    if fee is not None:
        try:
            return float(fee)
    ...
    addr_str = str(order.get("delivery_address") or "")
    if "Room" in addr_str and "," in addr_str:
        parts = [p.strip() for p in addr_str.split(",")]
        room_part = parts[0].replace("Room", "").strip()
        building_part = parts[1] if len(parts) > 1 else "AU 1"
        try:
            from routers.address import calculate_delivery_fee_details
            fee_details = calculate_delivery_fee_details(building_part, room_part)
            return float(fee_details["delivery_fee"])
        except Exception:
            pass
...
async def get_available_orders(rider_id: str = Depends(get_current_rider_id)):
    ...
    for item in orders_data:
        item_copy = dict(item)
        fee = get_order_delivery_fee(item_copy)
        item_copy["delivery_fee"] = fee
        item_copy["reward"] = f"₹{int(fee)}"
        mapped_orders.append(item_copy)
...
async def accept_order(order_id: str, rider_id: str = Depends(get_current_rider_id)):
    ...
    order_data = dict(res.data[0]) if res.data else {}
    fee = get_order_delivery_fee(order_data)
    order_data["delivery_fee"] = fee
    order_data["reward"] = f"₹{int(fee)}"
...
async def update_order_status(order_id: str, payload: StatusUpdate, rider_id: str = Depends(get_current_rider_id)):
    ...
    order_data = dict(res.data[0])
    earned_fee = get_order_delivery_fee(order_data)
    order_data["delivery_fee"] = earned_fee
    order_data["reward"] = f"₹{int(earned_fee)}"

    # If completed, add dynamic delivery fee to earnings and put rider back online
    if payload.status == "COMPLETED":
        rider_res = supabase.table("riders").select("earnings").eq("id", rider_id).execute()
        if rider_res.data:
            current_earnings = rider_res.data[0].get("earnings", 0)
            new_earnings = float(current_earnings or 0) + float(earned_fee)
...
async def get_rider_history(rider_id: str = Depends(get_current_rider_id)):
    ...
    for item in (res.data or []):
        ...
        item_copy["delivery_fee"] = fee
        item_copy["reward"] = f"₹{int(fee)}"
        mapped_history.append(item_copy)
