...
async def create_order(data: CreateOrderSchema, current_user_id: str = Depends(get_current_user_id)):
    ...
    for item in data.list_items:
        ...
        # Validate stock
        if prod_id:
            p_stock_res = supabase.table("products").select("stock").eq("id", prod_id).execute()
            if p_stock_res.data:
                current_stock = p_stock_res.data[0].get("stock", 0)
                if current_stock < item.quantity:
                    raise HTTPException(status_code=400, detail=f"Insufficient stock for product '{name}'. Available: {current_stock}")
                stock_updates.append({"id": prod_id, "new_stock": current_stock - item.quantity})
    ...
    try:
        order_res = supabase.table("orders").insert({**order_payload, "delivery_fee": delivery_fee}).execute()
    except Exception:
